...
import io.github.api.booking.fixture.CadastrarImovelRequestFixture;
...
class RealtyControllerIntegrationTest {
...
//        RestAssured.replaceFiltersWith(new RequestLoggingFilter(), new ResponseLoggingFilter());
    ...
    void deveRetornarBadRequestAoTentarCriarImovelSemCamposObrigatorios() {
        RegisterRealtyRequest registerRealtyRequest = CadastrarImovelRequestFixture.get()
                .semCamposObrigatorios()
                .build();

        given()
            .contentType(ContentType.JSON)
            .body(registerRealtyRequest)
        .when()
            .post(BASE_PATH)
        .then()
            .assertThat()
    ...
    void deveBuscarImovelPorIdComSucesso() {
        RegisterRealtyRequest registerRealtyRequest = CadastrarImovelRequestFixture.get().valido().build();
        ...
        .then()
            ...
            .statusCode(HttpStatus.OK.value())
            .body("id", equalTo(ID_IMOVEL_CRIADO))
            .body("tipoImovel", equalTo(registerRealtyRequest.getRealtyType().name()))
            .body("identificacao", equalTo(registerRealtyRequest.getIdentification()));
    }
